// Entrada unificada: teclado + ratón (con pointer lock) y táctil
// (joystick virtual a la izquierda, mirar arrastrando a la derecha).
...
export class Controls {
  constructor(canvas, ui, opts) {
    ...
    // --- ratón con pointer lock (escritorio) ---
    ...
    addEventListener('pointermove', (e) => {
      if (e.pointerId === this._stick.id) {
        ...
        this._stick.y = -dy / STICK_RADIUS; // arriba = avanzar
  ...
  // recalcula el vector de movimiento a partir de teclas + joystick
  updateMove() {
    let x = 0, z = 0;
    const k = this.keys;
    ...
    this.move.x = x;
    this.move.z = z;
    // en táctil, empujar el joystick a tope = esprintar
